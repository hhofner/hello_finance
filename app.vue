...
onMounted(async () => {
  ...
  if (error) {
    ...
    spentThisMonth.value = data.reduce((acc: number, expense: any) => acc + expense.price, 0)
  } else {
  ...
  <UContainer>
    <UCard class="mt-10">
      <template #header>
        ...
        <div>
          <p class="text-gray-500 flex items-center gap-1">This Month: <span>¥{{ spentThisMonth }}</span>
            <UIcon name="i-material-symbols-trending-up" class="w-5 h-5" />
          </p>
        </div>
      </template>
      <NuxtPage />
